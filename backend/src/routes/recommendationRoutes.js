...
const extractFallbackSource = (items = []) => items[0]?._fallbackSource || "unknown";

const sanitizeFallbackItems = (items = []) =>
  items.map(({ _fallbackSource, ...item }) => item);
...
router.get("/user/:userId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const fallbackSource = extractFallbackSource(fallbackRecommendations);

      return res.status(200).json({
        success: true,
        user_id: effectiveUserId,
        recommendations: sanitizeFallbackItems(fallbackRecommendations),
...
router.get("/similar/:animeId", async (req, res) => {
  try {
    ...
    const fallbackSource = extractFallbackSource(fallbackSimilar);

    return res.json({
      success: true,
      anime_id: req.params.animeId,
      similar: sanitizeFallbackItems(fallbackSimilar),
